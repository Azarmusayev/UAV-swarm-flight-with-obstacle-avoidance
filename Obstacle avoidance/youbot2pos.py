...
# robot goal - x pos, y pos, z orient
...
def odom_callback(msg):
    ...
    if not stop:
        #store robot x,y linear position
        ...
        #robot-                    x position, y postion, rotation w.r.t z-axis (deg)
        print('robot current position =', x_pos, y_pos, z_ornt * 180 / math.pi)

        #robot controller trigger every time ros subscriber callback is called
        move2goal(x_pos, y_pos, z_ornt)
...
def move2goal(curr_x, curr_y, curr_z):
    ...
    if not stop:
        ...
        #change in error difference & taking previous change as 0
        d_x = (e_x) / dt
        d_y = (e_y) / dt
        d_z = (e_z) / dt
        ...
        #publish on ros topic cmd_vel
        publish_on_vel(f_x, f_y, -f_z)
        ...
        #if goal reached than stop robot
...
def ros_init():
    ...
    rospy.init_node('move2pos', anonymous=True)

    rob_goal_pos[0] = input("Enter Goal X pos: ")
    rob_goal_pos[1] = input("Enter Goal Y pos: ")
    ornt_z_degree = input("Enter Goal z Orient(Deg): ")
    #ornt_z_degree = 0

    #degree to radian
    rob_goal_pos[2] = ornt_z_degree * math.pi / 180 + 5/6 * math.pi
